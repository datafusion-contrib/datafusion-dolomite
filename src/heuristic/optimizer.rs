...
impl HepOptimizer {
        ...
        match opt_expr.node() {
            ...
            OperatorNode(operator) => {
                ...
                for input_hep_node_id in input_hep_node_ids {
                    self.graph
                        .graph
                        .add_edge(new_node_id, input_hep_node_id, ());
                }
    ...
    fn init_with_plan(&mut self, plan: Plan) -> OptResult<()> {
        ...
        for plan_node_ref in plan
            .bfs_iterator()
            .collect::<Vec<PlanNodeRef>>()
            .into_iter()
            .rev()
        {
            parents.entry(plan_node_ref.id()).or_insert_with(Vec::new);
            for input in plan_node_ref.inputs() {
                parents
                    .get_mut(&plan_node_ref.id())
                    .unwrap()
                    .push(node_id_map.get(&input.id()).cloned().unwrap());
            }
            let plan_node_id =
                self.insert_opt_node(&OptExpression::<HepOptimizer>::with_operator(
                    plan_node_ref.operator().clone(),
                    parents
